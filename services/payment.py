...
class PaymentService:
  ...
  async def get_payment(self, payment_id: int, actor: CurrentUser) -> PaymentResponse:
    payment = await self.payment_repo.get_for_actor(
      payment_id=payment_id, actor_user_id=actor.user_id, role=actor.role
    )
    if payment is None:
  ...
  async def refund_payment(self, payment_id: int, actor: CurrentUser) -> PaymentResponse:
    ...
    if payment.status != PaymentStatus.SUCCEEDED:
      raise ServiceError(
        400, "INVALID_STATE", "Only succeeded payments can be refunded", actor.request_id
      )
  ...
  def _to_response(payment: Payment) -> PaymentResponse:
    method_value = (
      ...
      if isinstance(payment.payment_method, str)
      else getattr(payment.payment_method, "value", None)
    )
